...
// 随机获取ip
const getIp = () => _.random(1, 254) + '.' + _.random(1, 254) + '.' + _.random(1, 254) + '.' + _.random(1, 254)

// 休眠 如果需要间隔爬取使用
// const sleep = ms => new Promise(resolve => setTimeout(resolve, ms))

// 爬取函数
const reptile = (url, ip) =>
...
const _main = async path => {
    ...
    console.log('开始压缩')
    getZip(path)
    console.log('压缩成功,开始查重操作')
    const flight = fs.readFileSync(path)
    const flightData = JSON.parse(sky).flight
    const flightNo = []
    ...
    fs.writeFileSync('flight/flightNo.json', JSON.stringify(flightNo))
    console.log('查重结束,已写入flightNo.json,开始压缩')
    getZip('flight/flightNo.json')
    console.log('压缩成功,爬取成功结束')
}

_main('flight/flight.json')
